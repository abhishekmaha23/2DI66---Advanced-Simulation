...
def generatePick(possible_options):
...
def playGameConditional(n, start, p1_taken = [], p2_taken = []):
    possible_options = list(range(0,n**2))
...
def Simwin(possible_options, turn, taken1, taken2, amountOfRuns): # returns the best move in order to win
    ...
    for move in possible_options:
        ...
        drafts = 0
        for i in range(amountOfRuns):
            if playgame(move, taken1.copy(), taken2.copy(), turn) == 'p1':
                p1_wins +=1
            elif playgame(move, taken1.copy(), taken2.copy(), turn) =='p2':
                p2_wins +=1
            elif playgame(move, taken1.copy(), taken2.copy(), turn) =='draft':
                drafts += 1
        if turn == 'p1':
...
def playgame(start, taken1, taken2, turn): # simulates game given a certain move of a certain player, then continues with random placement of both players
    win_options = generateWinOptions(3)
    possible_options = list(range(0,3**2))
    possible_options = list(set(possible_options)-set(taken1)-set(taken2))
    if turn == 'p1':
        taken1.append(start)
    ...
    for move in possible_options:
        ...
        for i in range(amountOfRuns):
            if playgame(move, taken1.copy(), taken2.copy(), turn) == 'p1':
                ...
                drafts += 1
        if turn == 'p1':
            data[move] = max(p1_wins/amountOfRuns,drafts/amountOfRuns)
        elif turn == 'p2':
            data[move] = max(p2_wins/amountOfRuns,drafts/amountOfRuns)
    bestmove = max(data, key=lambda k: data[k])
    return bestmove


def Random(possible_options): # returns a random location on the board
    pick = possible_options[random.randint(0,len(possible_options)-1)]
    return pick
...
for strategy1 in ['Random','Simwin','SimNotLose']:
    for strategy2 in ['Random','Simwin','SimNotLose']:
        p1 = 0
        p2 = 0
        draft = 0
        for i in range(100):
            game = Question4(10000, strategy1, strategy2, 'p1', [], [])
            if game == 'p1': 
                p1 += 1
            elif game == 'p2':
                p2 += 1
